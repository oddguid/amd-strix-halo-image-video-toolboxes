...
These scripts ensure model files are downloaded to `~/comfy-ui/models/` where they survive toolbox refreshes. It will also set up essential ComfyUI extensions.

### 9.2. Run

The `start_comfy_ui` helper script starts ComfyUI with the `--base-path` set to your `~/comfy-ui` directory, where all extensions, models, output, and other user data will be stored, surviving toolbox refreshes.
...
```bash
cd /opt/ComfyUI
python main.py --port 8000 --base-directory "$HOME/comfy-ui" --disable-mmap --fp32-vae
```
